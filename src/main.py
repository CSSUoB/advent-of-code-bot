...
async def output_leaderboard(context, leaderboard_lst, title=None) -> None:
...
async def leaderboard(context, num_players: int = 20, year: int = CURRENT_YEAR, delta: bool = False) -> None:
    ...
    max_delta_mean_len = len(
        pretty_time(
            max(
                players,
                key=lambda t: len(pretty_time(t[6]["mean"]))
                if t[6]["mean"] is not None
                else 0,
            )[6]["mean"]
        )
    )
    max_delta_median_len = len(
        pretty_time(
            max(
                players,
                key=lambda t: len(pretty_time(t[6]["median"]))
                if t[6]["median"] is not None
                else 0,
            )[6]["median"]
        )
    )
    ...
    if not delta:
        for i, player in enumerate(players):
            if (
                player[2] != 0
                or datetime.datetime(CURRENT_YEAR, 12, 2) + datetime.timedelta(hours=5)
                > datetime.datetime.now()
            ):
                ranking.append(
                    PLAYER_STR_FORMAT.format(
                        rank=i + 1,
                        name=player[0],
                        name_pad=max_name_len,
                        points=player[1],
                        points_pad=max_points_len,
                        stars=player[2],
                        stars_pad=max_stars_len,
                        star_time=time.strftime(
                            "%H:%M %d/%m", time.localtime(player[3])
                        ),
                    )
                )
    else:
        ...
        for i, player in enumerate(players):
            if (player[6]["score"] == 0):  # minimum requirement for leaderboard is a gold star (score > 0)
                continue
            ranking.append(
                PLAYER_STR_FORMAT_DELTA.format(
                    rank=i + 1,
                    name=player[0],
                    name_pad=max_name_len,
                    delta_points=player[6]["score"],
                    delta_points_pad=max_delta_points_len,
                    stars=player[2],
                    stars_pad=max_stars_len,
                    delta_mean=pretty_time(player[6]["mean"]),
                    delta_mean_pad=max_delta_mean_len,
                    delta_median=pretty_time(player[6]["median"]),
                    delta_median_pad=max_delta_median_len,
                )
            )
    ...
    await output_leaderboard(
        ...
        title=f"{'Delta ' if delta else ''}Leaderboard for {year}:\n"
    )
...
async def rank(context, name: str, year: int = CURRENT_YEAR) -> None:
    ...
    if players:
        ...
        i, player = players[0]
        result = "```"
        result += PLAYER_STR_FORMAT.format(
            rank=i + 1,
            name=player[0],
            name_pad=len(player[0]),
            points=player[1],
            points_pad=len(str(player[1])),
            stars=player[2],
...
async def keen(context) -> None:
    ...
    players = [
        (i, player) for i, player in enumerate(all_players) if player[2] == max_stars
    ]
    ...
    if not delta:
        ...
        for player_day in players_day:
            if "1" in player_day[4][day]:
                first_star.append(
                    (player_day[0], int(player_day[4][day]["1"]["get_star_ts"]))
                )
            if "2" in player_day[4][day]:
                second_star.append(
                    (player_day[0], int(player_day[4][day]["2"]["get_star_ts"]))
                )
            ...
            for place, player in enumerate(final_table):
                ranking.append(
                    PLAYER_STR_FORMAT.format(
                        ...
                        stars_pad=max_stars_len,
                        star_time=time.strftime(
                            "%H:%M %d/%m", time.localtime(player[2])
                        ),
                    )
                )
    else:
                ...
                deltas.append(
                    {
                        "player": player,
                        "score": 0,
                        "delta": int(player[4][day]["2"]["get_star_ts"])
                        - int(player[4][day]["1"]["get_star_ts"]),
                    }
                )
        ...
        if deltas:
            ...
            for place, player in enumerate(deltas):
                ranking.append(
                    PLAYER_STR_FORMAT_DELTA_DAILY.format(
                        rank=place + 1,
                        name=player["player"][0],
                        name_pad=max_name_len,
                        delta_points=player["score"],
                        delta_points_pad=max_delta_points_len,
                        delta=pretty_time(player["delta"]),
                        delta_pad=max_delta_time_len,
                    )
                )
...
async def stars(context, day: Optional[str] = None, year: int = CURRENT_YEAR) -> None:
    ...
    if not final_table:
        ...
        for place, player in enumerate(final_table):
            leaderboard.append(
                PLAYER_STR_FORMAT_NOPOINTS.format(
                    rank=place + 1,
                    name=player[0],
                    name_pad=max_name_len,
                    points=player[1],
        ...
        await output_leaderboard(
            context=context,
            leaderboard_lst=leaderboard,
            title=f"Stars for day {day}, {year}:\n"
        )
